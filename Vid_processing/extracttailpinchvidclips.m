...
saveDir = fullfile( getrootdir, 'Data', 'FW_tail_pinch_vids' );
...
for expIdx = 1 : length( expList )
    ...
    mDatDir = fullfile( getrootdir, 'Data', mouse );
...
% Load existing key so as not to overwrite.
if exist( fullfile( getrootdir, 'Results', 'FW_tail_pinch_key.csv' ) )
    renameKeyOG = readtable(...
        fullfile( getrootdir, 'Results', 'FW_tail_pinch_key.csv' ),...
        'Delimiter', ',' );
else
    fprintf( 'FW_tail_pinch_key.csv does not exist yet.\n' )
end

vidListTmp = dir( saveDir );
...
writetable( renameKey,...
    fullfile( getrootdir, 'Results', 'FW_tail_pinch_key.csv' ) )
fprintf( 'Saved updated FW_tail_pinch_key.csv.\n')
